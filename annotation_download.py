...
def add_tator_getloc_args(parser):
    ...
    #parser.add_argumennt('--state-att', nargs=2, action='append', help='Further filter states by attribute equality')
...
if __name__=='__main__':
    ...
    if args.statetype:
        ...
        states = api.get_state_list(project_id, type=statetype.id, version=version_ids, media_id=media_ids) # todo section
        localizations = []
        media_frame_dict = defaultdict(list)
        for state in states:
            media_frame_dict[state.media[0]].append(state.frame)
        for media_id,frames in media_frame_dict.items():
            for frame in frames:
